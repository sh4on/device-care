...
3. If the internet is not working or user turns off mobile data/wifi, in this case, will the be cached and when internet is working or turn on wifi/mobile data, will it try to send again?
   ...
   Summary:
     ...
     - Calls / SMS → always in the phone log, ai fresh snapshot is sent on next successful cycle.
...
2. Check if it is present in firbase, if it present in firebase, then it will navigate to home screen but save it before.
